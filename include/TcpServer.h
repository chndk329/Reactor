...
private:
    ...
    std::vector<std::unique_ptr<EventLoop>> subloops_;     // 从事件循环，运行在线程池中
...
public:
    ...
    void newconnection(std::unique_ptr<Socket> clientsock);       // 创建客户端连上来的connection
    ...
    void sendcomplete(spConnection conn);            // 数据发送完成后，在Connecion类中回调此函数
    void epolltimeout(EventLoop *loop);     // epoll_wait()超时后，在EventLoop类中回调此函数

...
public:
    ...
    void HandleNewConnection(spConnection conn);                 // 创建新客户端的连接请求，在TcpServer类中回调此函数
    ...
    void HandleTimeOut(EventLoop *loop);                        // epoll_wait()超时后，在TcpServer类中回调此函数

    void OnMessage(spConnection conn, std::string &message);     // 处理客户端的请求报文（工作线程的任务函数）
};